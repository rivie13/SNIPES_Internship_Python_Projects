12/3/2024
updateMeraki_WAN_Traffic_Preferences
README.txt


Hello and thank you for using the updateMeraki_WAN_Traffic_Preferences application!
...
4. Check a store on Meraki to see if the flow preferences got updated correctly....
****************************************************************************************************************************************************************

Make sure to update the API key that is in the variableData file, if you don't then there could be errors

Sometimes for unknown reasons a 400 level error will occur when running the test usually along the lines of
'invalid API key' or 'unavailable resource' (not exactly like this but something along those lines...)
If these errors occur and you are sure you have a correct API key in the file, then you need to retry the program
...
open program that executable is based off of in vscode (updateMeraki_WAN_Traffic_Preferences.py)
open the terminal...
...
if the program runs properly then type the following command in the terminal:
     pyinstaller --onefile --paths='C:\Users\Riviera.Sperduto\OneDrive - JAKO ENTERPRISES LTD\Desktop\Meraki_Python_Programs\updateMeraki_WAN_Traffic_Preferences_V_0\source code\variableData.py' updateMeraki_WAN_Traffic_Preferences.py
    ...
    your --paths="" : YOUR PATH WILL BE DIFFERENT MAKE SURE TO GET YOUR PROPER PATH...
        ^^^ paths="" : the path where variable data file lives.... should be in same directory as the updateMeraki_WAN_Traffic_Preferences.py file..... if you right click on variableData.py and hit copy as path you will get the information you need!!!


If done correctly you should be able to look inside the updateMeraki_WAN_Traffic_Preferences directory with file explorer
...
1. open vscode click File menu tab and open the folder updateMeraki_WAN_Traffic_Preferences_V1_0 (might be different version now!)
2. open a terminal and type in the command: cd 'source code'      (include the single quotes!)
3. type in the command: python -m pytest test_updateMeraki_WAN_Traffic_Preferences -s
...
1. open vscode click File menu tab and open the folder updateMeraki_WAN_Traffic_Preferences_V1_0 (might be different version now!)
2. open a terminal and type in the command: cd 'source code'      (include the single quotes!)
3. type in the command: python updateMeraki_WAN_Traffic_Preferences.py
